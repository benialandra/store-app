        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-2 flex flex-col">
            ...
            <div className="flex items-center gap-6 text-sm text-[var(--muted)] font-medium mb-8">
              <span className="flex items-center gap-2"><Eye className="w-4 h-4" /> {product.views.toLocaleString()} views</span>
              <span className="flex items-center gap-2 text-amber-600 dark:text-amber-500"><ShoppingCart className="w-4 h-4" /> {product.sales.toLocaleString()} sold</span>
            </div>

            <div className="p-6 rounded-2xl border border-[var(--border)] bg-zinc-50 dark:bg-zinc-900/50 mb-6">
              <p className="text-sm text-[var(--muted)] mb-1">Price</p>
              <p className="text-3xl font-bold">Rp {product.price.toLocaleString('id-ID')}</p>
              <p className="text-sm text-[var(--muted)] mt-1">One-time payment · Lifetime license</p>
            </div>
